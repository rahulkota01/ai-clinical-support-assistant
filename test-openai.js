import { generateSafetyReport } from './services/openaiService.ts';
...
async function testOpenAISafetyReport() {
  console.log('Testing OpenAI generateSafetyReport function...');
  try {
    const report = await generateSafetyReport(testPatient);
    console.log('✅ OpenAI Safety report generated successfully!');
    console.log('Report length:', report.length);
    console.log('First 500 characters:');
    console.log(report.substring(0, 500) + '...');
  } catch (error) {
    console.error('❌ Error generating OpenAI safety report:', error.message);
  }
}

testOpenAISafetyReport();
